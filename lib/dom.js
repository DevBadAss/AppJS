...
class DOM {
     ...
     */
    selectall(query) {
        return document.querySelectorAll("." + query) || document.querySelectorAll("#" + query);
    }
    ...
    ChBgImage(iterator, images, element, speed) {
        setInterval(() => {
            this.setStyle(element, { backgroundImage: `url("${images[iterator]}")` });
            iterator++;
            if (iterator === images.length) {
                iterator = 0;
                this.ChBgImage(iterator, images, element, speed);
            }
        }, speed);
    }
}
